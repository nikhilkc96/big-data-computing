...
public class G26HW3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        JavaRDD<String> data = sc.textFile(file, L).cache();
        long read_time_end = System.currentTimeMillis();
        JavaRDD<Vector> inputPoints = data.map(
        ...
        for (int k = KStart; k <= KStart + H - 1; k++) {
            ...
            Broadcast<Map<Integer, Long>> sharedClusterSizes = sc.broadcast(currentClustering.map(x -> x._2).countByValue());

            Broadcast<List<Tuple2<Vector, Integer>>> clusteringSample = sc.broadcast(currentClustering.filter((x) -> {
                return rand.nextDouble() <= Math.min(M / sharedClusterSizes.getValue().get(x._2).doubleValue(), 1);
            }).collect());
